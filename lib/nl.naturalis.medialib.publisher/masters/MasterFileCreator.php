...
class MasterFileCreator {
	...
	private function _getAction($inputFile)
	{
		...
		if(!is_array($info)) {
			throw new Exception("Unable to retrieve image size for $inputFile");
		}
		$w = $info[0];
		$h = $info[1];
		if($w > $max || $h > $max) {
		...
		if($ext === 'jpg' || $ext === 'jpeg') {
			// The image is within the maximum bounds and it also already is a
			// jpeg image, so it should just be moved to the target directory.
			$this->_checkImage($inputFile);
			return self::ACTION_MOVE;
		}
	...
	private function _checkImage($file)
	{
		$commandLine = "identify -regard-warnings \"$file\"";
		$command = new Command($commandLine);
		if($command->execute() != 0) {
			throw new ImageMagickException($file, $command);
		}
	}
